...
- [Working with Multiple Datasets](#working-with-multiple-datasets) - Join datasets together  
- [Understanding Dataset Names](#understanding-nhanes-dataset-names) - Learn CDC naming conventions  
...
Quarterly checks ensure the package always has the most recently available NHANES data. If you notice an update before we do, we would love the feedback!

> **Note on Data Harmonization:** This package reconciles data types (integers and floats are now consistent) across NHANES cycles, but variable names remain unchanged from the original CDC data. Users should verify the CDC/NHANES documentation to understand how variable names and usage may have changed from cycle to cycle.
...
**Important Note on the `year` Variable:** The `year` column represents the **start year** of each 2-year survey cycle. For example, `year = 1999` indicates the 1999-2000 cycle, and `year = 2017` indicates the 2017-2018 cycle. Refer to the [NHANES documentation](https://www.cdc.gov/nchs/nhanes/) for more details on survey cycles and year representation.
...
Let's say you want to study hypertension but don't know which dataset contains blood pressure variables. **nhanesdata** provides search functions to help you discover variables.

### Method 1: Search by Term

Use `term_search()` to find variables by keyword:
...
This returns a data frame showing variable names, which datasets they appear in, and descriptions. For example, you'll see that `BPXSY1` (systolic blood pressure) is in the `BPX` tables, so you'd use `read_nhanes('bpx')`.

### Method 2: Search by Variable Name

If you know the exact variable name, use `var_search()`:
...
This shows which survey cycles contain that variable, which is helpful for understanding data availability over time.
...
All search functions are **case-insensitive**, so `term_search()`, `var_search()`, and other functions work with any capitalization.

## Working with Multiple Datasets

The real power of NHANES comes from linking datasets together. You can join data from different examinations and questionnaires using the `seqn` (participant ID) and `year` (survey cycle) columns.
...
**Key joining principle:** Always join on both `seqn` (participant ID) AND `year` (survey cycle). The same participant can appear in multiple survey cycles, and the same sequence number can be reused across cycles.

## Data Storage Format

All datasets are stored remotely as `.parquet` files for optimal performance and cross-platform compatibility. **Note:** The storage format does not affect how you read the data - simply use `read_nhanes()` and the function handles everything. You are not writing `.parquet` files to your local environment unless you explicitly choose to save them locally.
