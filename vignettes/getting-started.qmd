...
Here's where the CDC's naming system can trip you up. When you use `read_r2('demo')`, you're getting data that the CDC publishes across multiple tables:
...
From this, you can see that blood pressure data lives in the `BPX` tables. In nhanesdata, that means you want `read_r2('bpx')`.
...
### Method 3: Find Variables in Loaded Data

If you've already loaded several datasets and can't remember which one contains a variable:
...
# Which dataset has the height variable?
find_variable('bmxht')
```

This will tell you that `bmxht` (height) is in the `bmx` dataset.
...
## Advanced: Pulling Fresh Data from CDC

Most users will be well-served by `read_r2()`, which provides pre-processed, stable data updated quarterly. However, if you need:

- Custom variable selection (only specific columns)
- The absolute latest data before our quarterly update
- To build your own data pipeline

You can use `pull_nhanes()` to download directly from the CDC:

```{r}
#| label: pull-fresh
#| eval: false

# Pull only specific variables from the demographics data
demo_custom <- pull_nhanes(
  'demo',
  selected_variables = c('SEQN', 'RIDAGEYR', 'RIAGENDR', 'RIDRETH1'),
...
- This function downloads from CDC servers, which can be slow or fail
- It merges all cycles and harmonizes data types automatically
- Use `save = TRUE` if you want to cache the results locally
- The resulting data structure is identical to what `read_r2()` provides

For most analyses, stick with `read_r2()` for reliability and speed.

## Summary: What You've Learned

In this vignette, you've learned how to:

1. Load NHANES datasets instantly with `read_r2()`
...
4. Find variables using `term_search()`, `var_search()`, and `find_variable()`
...
All of these datasets are available through `read_r2()`. Use the search functions to explore what's available, and don't hesitate to combine multiple datasets to answer complex questions.
