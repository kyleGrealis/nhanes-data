...
An R package for accessing NHANES (National Health and Nutrition
Examination Survey) data with automated quarterly updates from CDC
servers. Automatically harmonizes multi-cycle data, handles type
inconsistencies across survey years, and provides pre-processed datasets
for fast access.
...
1.  **Public Data Access**: Pre-processed datasets hosted on Cloudflare
    R2 for fast, reliable downloads
2.  **Data Harmonization**: Seamlessly combines survey cycles
    (1999-2021) with automatic type reconciliation
3.  **Storage Format**: Saves data `.parquet` (cross-platform) for fast
    reading and importing
4.  **Change Detection**: MD5 checksums ensure only updated datasets are
...
**All functions are case-insensitive** - use ‘demo’, ‘DEMO’, or ‘Demo’
interchangeably!
...
Data is updated quarterly via automated workflows. See the workflow
badge at the top for latest update status.
...
This package includes a **fully automated GitHub Actions workflow**
that:

1.  **Runs quarterly** (Jan 1, Apr 1, Jul 1, Oct 1 at 2 AM UTC)
2.  **Pulls fresh data** from CDC servers for all NHANES datasets
...
The automation processes **Currently includes 66 datasets (with more
coming)** across two categories:
...
- **survey** / **srvyr**: Complex survey analysis (not used in data
  acquisition)  
...
This pattern applies to **all 66 datasets**: `BPX`, `BPX_B`, `BPX_C`, …,
`BPX_L` for blood pressure examination data, and so on.
...
suffix codes) - Enables longitudinal analysis (all years in one
dataset) - Handles type mismatches between cycles automatically - Adds
temporal context via the `year` column
...
Since the `year` column tracks survey cycles, you can filter and analyze
by time period:
...
The package uses **parquet format** for optimal performance and
cross-platform compatibility:

- **`.parquet`**: Cross-platform, fast I/O, columnar storage, works with
  Python/Julia/Arrow

The
[`read_nhanes()`](https://kyleGrealis.com/nhanesdata/reference/read_nhanes.md)
function uses `.parquet` files for better performance and
interoperability.
...
To push processed data to your own R2 bucket, configure these
...
See `SECURITY.md` for detailed setup instructions and security best
practices.
...
The workflow at `.github/workflows/update-nhanes-data.yml` will use
these automatically.
...
- **[nhanesA](https://cran.r-project.org/package=nhanesA)**: Direct
  interface to NHANES API (this package wraps it for multi-cycle
  harmonization)  
