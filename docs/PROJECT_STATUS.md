...
**Current Version:** 0.0.0.9000 (development) **Last Updated:**
2025-11-19
...
**v0.0.0.9000 (development):** - ✅ Renamed `read_r2()` →
[`read_nhanes()`](https://kyleGrealis.com/nhanesdata/reference/read_nhanes.md)
for better clarity - ✅ Made `pull_nhanes()` internal-only (maintainer
use) - ✅ Optimized dependencies (7 packages moved to Suggests) - ✅
Added comprehensive documentation with case-insensitivity examples - ✅
Added COVID-19 cycle documentation - ✅ Acknowledged nhanesA package
developers
...
and `.get_year_from_suffix()`

Rename `test-read_r2.R` → `test-read-nhanes.R`

### Future Improvements

Consider inlining workflow helper functions (detect_data_changes,
update_checksum, load_dataset_config) directly into workflow_update.R
...
**Current Coverage:** ~70% (pragmatic approach) - ✅ Excellent coverage
for checksums and config loading - ✅ Mock tests for read_nhanes - ⚠️
Missing tests for get_url (pure logic, should have tests) - ⚠️ Skipping
tests for thin wrappers (term_search, var_search)
