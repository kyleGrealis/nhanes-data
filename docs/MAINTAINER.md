...
Settings → Secrets and variables → Actions → Add: - `R2_ACCOUNT_ID` -
...
1.  `pull_nhanes(dataset)` - Downloads from CDC, merges cycles, returns
    data
2.  `nhanes_pin_write(data, name, bucket)` - Uploads to R2 (pinning is
    automatic)
